...
contract Cookie is ERC20("Cookie", "COOKIE"), Ownable {
    ...
    function setBakeryAddress(address _bakeryAddress) external onlyOwner {
        require(
            address(bakeryAddress) == address(0),
            "Bakery address already set"
        );
    ...
    function setBakerAddress(address _bakerAddress) external onlyOwner {
        require(
            address(bakerAddress) == address(0),
            "Baker address already set"
        );
    ...
    function mint(address _to, uint256 _amount) external {
        require(
            _msgSender() == bakeryAddress,
            "Only the Bakery contract can mint"
        );
        _mint(_to, _amount);
    }
    ...
    function burn(address _from, uint256 _amount) external {
        require(
            _msgSender() == bakerAddress,
            "Only the Baker contract can burn"
        );
        _burn(_from, _amount);
    }
    ...
    function transferToPantry(address _from, uint256 _amount) external {
        require(
            _msgSender() == pantryAddress,
            "Only the Pantry contract can call transferToPantry"
        );
...
contract Baker is ERC721Enumerable, Ownable {
    ...
    function mintBase(uint16 _numTokens) external payable {
        require(
            msg.value == _numTokens * BASE_MINT_PRICE,
    ...
    function presaleMintBase(uint16 _numTokens) external payable {
        require(
            msg.value == _numTokens * PRESALE_MINT_PRICE,
            "Incorrect amount sent"
        );
        require(presaleOpen(), "The presale is not open");
        require(
            presaleSupply + _numTokens <= MAX_PRESALE_SUPPLY,
    ...
    function _mintBaseTokens(uint16 _numTokens, address _for) internal {
        require(
            baseSupply + _numTokens <= MAX_BASE_SUPPLY,
            "Insufficient supply"
        );
    ...
    function mintUpgrade(uint256 _level, uint16 _numTokens) external {
        ...
        require(
            levels[_level].supply + _numTokens <= levels[_level].maxSupply,
            "Insufficient supply"
        );

        uint256 totalCost = _numTokens * levels[_level].price;
        require(
            cookie.balanceOf(msg.sender) >= totalCost,
            "Insufficient COOKIE balance"
        );
    ...
    function gameStarted() public view returns (bool) {
        return
            baseSalesOpen() &&
            (block.timestamp >= salesStartTime + UPGRADE_SALES_OFFSET ||
                baseSupply == MAX_BASE_SUPPLY);
    }
    ...
    function tokenRevealed(uint256 _tokenId) public view returns (bool) {
        return
            isUpgrade(_tokenId) ||
            baseTokenMintBlock[_tokenId] < block.number - 1;
    }

    function revealedTokenLevel(uint256 _tokenId)
        public
        view
        returns (uint256)
    {
            ...
            return
                string(
                    abi.encodePacked(
                        BASE_URI,
                        "level/",
                        revealedTokenLevel(_tokenId).toString(),
                        ".json"
                    )
                );
...
contract Bakery is Ownable {
    ...
    function getCookiesAccruedForMany(uint256[] calldata _tokenIds)
        external
        view
    ...
    function _getCookiesAccruedFor(uint256 _tokenId, bool checkOwnership)
        internal
        view
        returns (uint256)
    {
    ...
    function stakeMany(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                baker.ownerOf(tokenId) == _msgSender(),
    ...
    function claimCookiesAndMaybeUnstake(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            uint256 taxAmount = (cookies * CLAIM_COOKIE_TAX_PERCENTAGE + 99) /
